...
export function ImportCactbotDialog({ onClose }: Props) {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      setContent(text);
      // Pre-fill id/name from file content header (### BOSS NAME)
  ...
  async function handleImport() {
    ...
    try {
      ...
      setStatus({ type: "success", msg: `「${timeline.name}」を取り込みました (${timeline.events.length}イベント)` });
    } catch (e) {
      ...
      >
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            placeholder="例: Eden's Promise (Eternity) Savage"
            disabled={busy}
          />
        </div>
